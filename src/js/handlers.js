...
const handlers = {
  ...
  addCommas(input) {
    ...
    if (input.includes(".")) {
      ...
      afterDecimalPortionOfValue =
        afterDecimalPortionOfValue === "."
          ? "."
          : Number(afterDecimalPortionOfValue)
              .toFixed(afterDecimalPortionOfValue.length > 2 ? 2 : 1)
              .toString()
              .replace(/^0/, "");
    }
    ...
    return (valueWithCommas  || input) + (afterDecimalPortionOfValue || "");
  },
  onDigitPress(input, buttonPressed) {
    if ((input === "0" && buttonPressed !== ".") || state.operatorLastPressed) {
      state.operatorLastPressed = false;
